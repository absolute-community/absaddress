(function (ninja) {
	var ut = ninja.unitTests = {
		runSynchronousTests: function (showOutput) {
			...
			var userKeyPool = Absolute.KeyPool.getArray(); // get the user key pool before test keys get added to it
			var testResults = "";
			var passCount = 0;
			var testCount = 0;
			...
			Absolute.KeyPool.setArray(userKeyPool); // set the key pool so users don't see the test keys
		...
		runAsynchronousTests: function (showOutput) {
			...
			var userKeyPool = Absolute.KeyPool.getArray();
			...
			ninja.foreachSerialized(ninja.unitTests.asynchronousTests, function (name, cb) {
				...
				Absolute.KeyPool.setArray(userKeyPool);
				document.getElementById("busyblock").className = "";
			});
		},
		...
		synchronousTests: {
			...
			testGetAbsoluteAddressFromByteArray: function () {
				var bytes = [4, 120, 152, 47, 64, 250, 12, 11, 122, 85, 113, 117, 131, 175, 201, 154, 78, 223, 211, 1, 162, 114, 157, 197, 155, 11, 142, 185, 225, 134, 146, 188, 181, 33, 240, 84, 250, 217, 130, 175, 76, 193, 147, 58, 253, 31, 27, 86, 62, 167, 121, 166, 170, 108, 206, 54, 163, 11, 148, 125, 214, 83, 230, 62, 68];
				var address = ninja.publicKey.getAbsoluteAddressFromByteArray(bytes);
			...
			testGetByteArrayFromMultiplying: function () {
				...
				var bytes = ninja.publicKey.getByteArrayFromMultiplying(key1, new Absolute.ECKey(key2));
			...
			testGetByteArrayFromMultiplyingCompressedOutputsCompressed: function () {
				...
				var ecKey = new Absolute.ECKey(key2);
			...
			testGetPubHexFromMultiplyingPrivAPubB: function () {
				...
				var bytes = ninja.publicKey.getByteArrayFromMultiplying(keyPub, new Absolute.ECKey(keyPriv));
			...
			testBadKeyIsNotWif: function () {
				return !(Absolute.ECKey.isWalletImportFormat("bad key"));
			},
			testBadKeyIsNotWifCompressed: function () {
				return !(Absolute.ECKey.isCompressedWalletImportFormat("bad key"));
			},
			testBadKeyIsNotHex: function () {
				return !(Absolute.ECKey.isHexFormat("bad key"));
			},
			testBadKeyIsNotBase64: function () {
				return !(Absolute.ECKey.isBase64Format("bad key"));
			},
			testBadKeyIsNotMini: function () {
				return !(Absolute.ECKey.isMiniFormat("bad key"));
			},
			testBadKeyReturnsNullPrivFromECKey: function () {
				var key = "bad key";
				var ecKey = new Absolute.ECKey(key);
				if (ecKey.priv != null) {
					return false;
				}
				return true;
			},
			testGetAbsolutePrivateKeyByteArray: function () {
				...
				var btcKey = new Absolute.ECKey(key);
				if (btcKey.getAbsolutePrivateKeyByteArray().toString() != bytes.toString()) {
					return false;
				}
				return true;
			},
			testECKeyDecodeWalletImportFormat: function () {
				...
				var bytes2 = Absolute.ECKey.decodeWalletImportFormat(key);
			...
			testECKeyDecodeCompressedWalletImportFormat: function () {
				...
				var bytes1 = [41, 38, 101, 195, 135, 36, 24, 173, 241, 218, 127, 250, 58, 100, 111, 47, 6, 2, 36, 109, 166, 9, 138, 145, 210, 41, 195, 33, 80, 242, 113, 139];
				var bytes2 = Absolute.ECKey.decodeCompressedWalletImportFormat(key);
				if (bytes1.toString() != bytes2.toString()) {
					return false;
				}
				return true;
			...
			testBase64ToECKey: function () {
				...
				var btcKey = new Absolute.ECKey(key);
				if (btcKey.getAbsoluteBase64Format() != "KSZlw4ckGK3x2n/6OmRvLwYCJG2mCYqR0inDIVDycYs=") {
					return false;
				}
				return true;
			},
			testHexToECKey: function () {
				...
				var btcKey = new Absolute.ECKey(key);
				if (btcKey.getAbsoluteHexFormat() != "292665C3872418ADF1DA7FFA3A646F2F0602246DA6098A91D229C32150F2718B") {
					return false;
				}
				return true;
			},
			testCompressedWifToECKey: function () {
				var key = "KxbhchnQquYQ2dfSxz7rrEaQTCukF4uCV57TkamyTbLzjFWcdi3S";
				var btcKey = new Absolute.ECKey(key);
				if (btcKey.getAbsoluteWalletImportFormat() != "KxbhchnQquYQ2dfSxz7rrEaQTCukF4uCV57TkamyTbLzjFWcdi3S"
						|| btcKey.getPubPoint().compressed != true
			...
			testWifToECKey: function () {
				var key = "5J8QhiQtAiozKwyk3GCycAscg1tNaYhNdiiLey8vaDK8Bzm4znb";
				var btcKey = new Absolute.ECKey(key);
				if (btcKey.getAbsoluteWalletImportFormat() != "5J8QhiQtAiozKwyk3GCycAscg1tNaYhNdiiLey8vaDK8Bzm4znb"
					|| btcKey.compressed == true) {
					return false;
				}
				return true;
			...
			testBrainToECKey: function () {
				...
				var btcKey = new Absolute.ECKey(bytes);
				if (btcKey.getAbsoluteWalletImportFormat() != "5J8QhiQtAiozKwyk3GCycAscg1tNaYhNdiiLey8vaDK8Bzm4znb") {
					return false;
				}
				return true;
			...
			testMini30CharsToECKey: function () {
				...
				var btcKey = new Absolute.ECKey(key);
				if (btcKey.getAbsoluteWalletImportFormat() != "5JrBLQseeZdYw4jWEAHmNxGMr5fxh9NJU3fUwnv4khfKcg2rJVh") {
					return false;
				}
				return true;
			},
			testGetECKeyFromAdding: function () {
				...
				if (ecKey.getAbsoluteWalletImportFormat() != "5KAJTSqSjpsZ11KyEE3qu5PrJVjR4ZCbNxK3Nb1F637oe41m1c2") {
					return false;
				}
				return true;
			},
			testGetECKeyFromAddingCompressed: function () {
				...
				var key2 = "L1n4cgNZAo2KwdUc15zzstvo1dcxpBw26NkrLqfDZtU9AEbPkLWu";
				var ecKey = ninja.privateKey.getECKeyFromAdding(key1, key2);
				if (ecKey.getAbsoluteWalletImportFormat() != "L3A43j2pc2J8F2SjBNbYprPrcDpDCh8Aju8dUH65BEM2r7RFSLv4") {
					return false;
				}
				return true;
			},
			...
			testGetECKeyFromMultiplying: function () {
				...
				if (ecKey.getAbsoluteWalletImportFormat() != "5KetpZ5mCGagCeJnMmvo18n4iVrtPSqrpnW5RP92Gv2BQy7GPCk") {
					return false;
				}
				return true;
			},
			testGetECKeyFromMultiplyingCompressed: function () {
				...
				var key2 = "L1n4cgNZAo2KwdUc15zzstvo1dcxpBw26NkrLqfDZtU9AEbPkLWu";
				var ecKey = ninja.privateKey.getECKeyFromMultiplying(key1, key2);
				if (ecKey.getAbsoluteWalletImportFormat() != "L5LFitc24jme2PfVChJS3bKuQAPBp54euuqLWciQdF2CxnaU3M8t") {
					return false;
				}
				return true;
			},
			...
			testGetECKeyFromBase6Key: function () {
				...
				var ecKey = new Absolute.ECKey(baseKey);
				if (ecKey.getAbsoluteHexFormat() != hexKey) {
					return false;
				}
				return true;
			},
			...
			testBugWithLeadingZeroBytePublicKey: function () {
				...
				var btcKey = new Absolute.ECKey(key);
				if (btcKey.getAbsoluteAddress() != "1M6dsMZUjFxjdwsyVk8nJytWcfr9tfUa9E") {
					return false;
				}
				return true;
			},
			testBugWithLeadingZeroBytePrivateKey: function () {
				...
				var btcKey = new Absolute.ECKey(key);
				if (btcKey.getAbsoluteAddress() != "1NAjZjF81YGfiJ3rTKc7jf1nmZ26KN7Gkn") {
					return false;
				}
				return true;
			},
			...
			testSplitAndCombinePrivateKey2of2: function () {
				...
				var btcKey = new Absolute.ECKey(combined);

				if (btcKey.getAbsoluteHexFormat() != key.toUpperCase()) {
					return false;
				}
				return true;
			},
			...
			testSplitAndCombinePrivateKey2of4: function () {
				...
				if (btcKey.getAbsoluteHexFormat() != key) {
					return false;
				}
				return true;
			},
			...
			testCombinePrivateKeyFromXofYShares: function () {
				...
				var combined = ninja.wallets.splitwallet.combineFormattedShares(shares);
				var btcKey = new Absolute.ECKey(combined);
				if (btcKey.getAbsoluteWalletImportFormat() != key) {
					return false;
				}
				return true;
			},

			//Absolute.KeyPool tests
			testKeyPoolStoresCompressedAndUncompressedKey: function () {
				...
				var btcKeyUncompressed = new Absolute.ECKey(keyUncompressed);
				var btcKeyCompressed = new Absolute.ECKey(keyCompressed);
				var pool = Absolute.KeyPool.getArray();
				
				if (pool.length != 2
					|| pool[0].getAbsoluteWalletImportFormat() != keyUncompressed
					|| pool[1].getAbsoluteWalletImportFormat() != keyCompressed
				) {
					return false;
				}
				return true;
			...
			testKeyPoolPreventDuplicatesWhenAdding: function () {
				...
				var btcKeyUncompressed = new Absolute.ECKey(keyUncompressed);
				var btcKeyCompressed = new Absolute.ECKey(keyCompressed);
				var btcKeyCompressed2 = new Absolute.ECKey(keyCompressed);
				var btcKeyUncompressed2 = new Absolute.ECKey(keyUncompressed);
				var btcKeyHex = new Absolute.ECKey(keyHex);
				var pool = Absolute.KeyPool.getArray();
		...
		asynchronousTests: {
			//https://en.absolute.it/wiki/BIP_0038
			testBip38: function (done) {
				...
				var decryptBip38Test = function (test, i, onComplete) {
					ninja.privateKey.BIP38EncryptedKeyToByteArrayAsync(test[0], test[1], function (privBytes) {
						if (privBytes.constructor == Error) {
							...
							var btcKey = new Absolute.ECKey(privBytes);
							var wif = !test[2].substr(0, 1).match(/[LK]/) ? btcKey.setCompressed(false).getAbsoluteWalletImportFormat() : btcKey.setCompressed(true).getAbsoluteWalletImportFormat();
							if (wif != test[2]) {
				...
				var cycleBip38Test = function (i, compress, onComplete) {
					...
					var privKey = (new Absolute.ECKey(false)).getAbsoluteWalletImportFormat();
					...
					ninja.privateKey.BIP38PrivateKeyToEncryptedKeyAsync(privKey, 'testing', compress, function (encryptedKey) {
						...
						ninja.privateKey.BIP38EncryptedKeyToByteArrayAsync(encryptedKey, 'testing', function (decryptedBytes) {
							var decryptedKey = (new Absolute.ECKey(decryptedBytes)).getAbsoluteWalletImportFormat();
						...
						ninja.privateKey.BIP38GenerateECAddressAsync(intermediatePoint, false, function (address, encryptedKey) {
							ninja.privateKey.BIP38EncryptedKeyToByteArrayAsync(encryptedKey, pass, function (privBytes) {
								if (privBytes.constructor == Error) {
									...
									var btcKey = new Absolute.ECKey(privBytes);
									var btcAddress = btcKey.getAbsoluteAddress();
									if (address !== btcKey.getAbsoluteAddress()) {
