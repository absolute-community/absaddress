ninja.wallets.vanitywallet = {
	...
	generateKeyPair: function () {
		...
		var privateKey = key.getAbsoluteHexFormat();
	...
	addKeys: function () {
		...
		var absoluteAddress = ninja.translator.get("vanityinvalidinputcouldnotcombinekeys");
		...
		try {
			...
			if (ninja.publicKey.isPublicKeyHexFormat(input1KeyString) && ninja.publicKey.isPublicKeyHexFormat(input2KeyString)) {
				...
				if (document.getElementById("vanityradioadd").checked) {
					...
					if (pubKeyByteArray == null) {
						...
						absoluteAddress = ninja.publicKey.getAbsoluteAddressFromByteArray(pubKeyByteArray);
			...
			// one public key and one private key
				...
				var ecKey = (ninja.privateKey.isPrivateKey(input1KeyString)) ? new Absolute.ECKey(input1KeyString) : new Absolute.ECKey(input2KeyString);
				// add 
				if (document.getElementById("vanityradioadd").checked) {
				...
				if (pubKeyCombined == null) {
					...
					absoluteAddress = ninja.publicKey.getAbsoluteAddressFromByteArray(pubKeyCombined);
			...
			// both inputs are private keys
				...
				if (combinedPrivateKey == null) {
					...
					absoluteAddress = combinedPrivateKey.getAbsoluteAddress();
					privateKeyWif = combinedPrivateKey.getAbsoluteWalletImportFormat();
		...
		document.getElementById("vanityaddress").innerHTML = absoluteAddress;
