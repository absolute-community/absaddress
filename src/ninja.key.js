...
ninja.privateKey = {
	isPrivateKey: function (key) {
		return (
					Absolute.ECKey.isWalletImportFormat(key) ||
					Absolute.ECKey.isCompressedWalletImportFormat(key) ||
					Absolute.ECKey.isHexFormat(key) ||
					Absolute.ECKey.isBase64Format(key) ||
					Absolute.ECKey.isMiniFormat(key)
				);
	},
	getECKeyFromAdding: function (privKey1, privKey2) {
		var n = EllipticCurve.getSECCurveByName("secp256k1").getN();
		var ecKey1 = new Absolute.ECKey(privKey1);
		var ecKey2 = new Absolute.ECKey(privKey2);
		// if both keys are the same return null
		if (ecKey1.getAbsoluteHexFormat() == ecKey2.getAbsoluteHexFormat()) return null;
		if (ecKey1 == null || ecKey2 == null) return null;
		var combinedPrivateKey = new Absolute.ECKey(ecKey1.priv.add(ecKey2.priv).mod(n));
	...
	getECKeyFromMultiplying: function (privKey1, privKey2) {
		...
		if (ecKey1.getAbsoluteHexFormat() == ecKey2.getAbsoluteHexFormat()) return null;
		if (ecKey1 == null || ecKey2 == null) return null;
		var combinedPrivateKey = new Absolute.ECKey(ecKey1.priv.multiply(ecKey2.priv).mod(n));
		// compressed when both keys are compressed
		if (ecKey1.compressed && ecKey2.compressed) combinedPrivateKey.setCompressed(true);
		return combinedPrivateKey;
	},
	...
	BIP38EncryptedKeyToByteArrayAsync: function (base58Encrypted, passphrase, callback) {
		var hex;
		try {
			hex = Absolute.Base58.decode(base58Encrypted);
		} catch (e) {
		...
		hex = hex.slice(0, -4);
		var checksum = Absolute.Util.dsha256(hex);
		...
		var verifyHashAndReturn = function () {
			var tmpkey = new Absolute.ECKey(decrypted); // decrypted using closure
			var base58AddrText = tmpkey.setCompressed(isCompPoint).getAbsoluteAddress(); // isCompPoint using closure
			checksum = Absolute.Util.dsha256(base58AddrText); // checksum using closure

			if (checksum[0] != hex[3] || checksum[1] != hex[4] || checksum[2] != hex[5] || checksum[3] != hex[6]) {
				callback(new Error(ninja.translator.get("bip38alertincorrectpassphrase"))); // callback using closure
				return;
			}
			callback(tmpkey.getAbsolutePrivateKeyByteArray()); // callback using closure
		};
		...
		if (!isECMult) {
			...
			Crypto_scrypt(passphrase, ownersalt, 16384, 8, 8, 32, function (prefactorA) {
				...
				Crypto_scrypt(passpoint, addresshashplusownerentropy, 1024, 1, 1, 64, function (derived) {
					...
					var factorb = Absolute.Util.dsha256(seedb);
	...
	BIP38PrivateKeyToEncryptedKeyAsync: function (base58Key, passphrase, compressed, callback) {
		var privKey = new Absolute.ECKey(base58Key);
		var privKeyBytes = privKey.getAbsolutePrivateKeyByteArray();
		var address = privKey.setCompressed(compressed).getAbsoluteAddress();

		// compute sha256(sha256(address)) and take first 4 bytes
		var salt = Absolute.Util.dsha256(address).slice(0, 4);
		...
		Crypto_scrypt(passphrase, salt, 16384, 8, 8, 64, function (derivedBytes) {
			...
			encryptedKey = encryptedKey.concat(Absolute.Util.dsha256(encryptedKey).slice(0, 4));
			callback(Absolute.Base58.encode(encryptedKey));
		});
	},
	BIP38GenerateIntermediatePointAsync: function (passphrase, lotNum, sequenceNum, callback) {
		...
		Crypto_scrypt(passphrase, ownerSalt, 16384, 8, 8, 32, function (prefactor) {
			...
			var passfactorBytes = noNumbers ? prefactor : Absolute.Util.dsha256(prefactor.concat(ownerEntropy));
			...
			// base58check encode
			intermediate = intermediate.concat(Absolute.Util.dsha256(intermediate).slice(0, 4));
			callback(Absolute.Base58.encode(intermediate));
		});
	},
	BIP38GenerateECAddressAsync: function (intermediate, compressed, callback) {
		// decode IPS
		var x = Absolute.Base58.decode(intermediate);
		//if(x.slice(49, 4) !== Absolute.Util.dsha256(x.slice(0,49)).slice(0,4)) {
		...
		// 3) ECMultiply passpoint by factorb. Use the resulting EC point as a public key and hash it into a Absolute
		// address using either compressed or uncompressed public key methodology (specify which methodology is used
		// inside flagbyte). This is the generated Absolute address, call it generatedaddress.
		...
		var generatedAddress = (new Absolute.Address(Absolute.Util.sha256ripe160(generatedBytes))).toString();

		// 4) Take the first four bytes of SHA256(SHA256(generatedaddress)) and call it addresshash.
		var addressHash = Absolute.Util.dsha256(generatedAddress).slice(0, 4);
		...
		Crypto_scrypt(passpoint, addressHash.concat(ownerEntropy), 1024, 1, 1, 64, function (derivedBytes) {
			...
			// base58check encode
			encryptedKey = encryptedKey.concat(Absolute.Util.dsha256(encryptedKey).slice(0, 4));
			callback(generatedAddress, Absolute.Base58.encode(encryptedKey));
		});
	}
};
...
ninja.publicKey = {
	...
	getAbsoluteAddressFromByteArray: function (pubKeyByteArray) {
		var pubKeyHash = Absolute.Util.sha256ripe160(pubKeyByteArray);
		var addr = new Absolute.Address(pubKeyHash);
