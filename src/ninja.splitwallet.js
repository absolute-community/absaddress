ninja.wallets.splitwallet = {
	...
	splitKey: function () {
		try {
			...
			var shares = ninja.wallets.splitwallet.getFormattedShares(key.getAbsoluteHexFormat(), numshares, threshold);

			var output = document.createElement("div");
			output.setAttribute("id", "splitoutput");
			var m = {};
			output.appendChild(this.mkOutputRow(absoluteAddress, "split_addr", "Absolute Address:    "));
			m["split_addr"] = absoluteAddress;
	...
	combineShares: function () {
		try {
			...
			var privkeyBase58 = new Absolute.ECKey(combinedBytes).getAbsoluteWalletImportFormat();
	...
	getFormattedShares: function (key, numshares, threshold) {
		var shares = secrets.share(key, numshares, threshold).map(ninja.wallets.splitwallet.hexToBytes).map(Absolute.Base58.encode);
		return shares;
	},

	// combine base58 formatted shares and return a absolute byte array
	combineFormattedShares: function (shares) {
		var combined = secrets.combine(shares.map(Absolute.Base58.decode).map(Crypto.util.bytesToHex).map(ninja.wallets.splitwallet.stripLeadZeros));
